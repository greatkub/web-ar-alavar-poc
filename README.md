...
The browser segmentation test route is available from the Home screen and directly at `/?route=sam3-litetext` or `/sam3-litetext`. It now runs on the client with `@huggingface/transformers` and `Xenova/segformer-b0-finetuned-ade-512-512`, using WebGPU when `navigator.gpu` is available on a secure origin and falling back to the browser CPU/WASM runtime otherwise. The selected photo is resized in-browser before inference, the prompt filters fixed semantic labels such as `tree`, `plant`, `grass`, `flower`, and `palm`, and masks are shown as an overlay plus compact uncompressed RLE in the JSON output. Override the browser model with `VITE_BROWSER_SEGMENTATION_MODEL`, force a runtime with `VITE_BROWSER_SEGMENTATION_DEVICE` (`auto`, `webgpu`, or `wasm`; `wasm` maps to Transformers.js `cpu`), tune image upload size with `VITE_BROWSER_SEGMENTATION_IMAGE_MAX_WIDTH` and `VITE_BROWSER_SEGMENTATION_IMAGE_JPEG_QUALITY`, and adjust pipeline defaults with `VITE_BROWSER_SEGMENTATION_THRESHOLD` and `VITE_BROWSER_SEGMENTATION_MASK_THRESHOLD`. This route no longer calls the Railway SAM3-LiteText API by default.
