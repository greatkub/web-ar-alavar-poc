...
The SAM3-LiteText test route is available from the Home screen and directly at `/?route=sam3-litetext` or `/sam3-litetext`. It sends the selected photo and text prompt to `https://web-ar-alavar-poc-fastapi-production.up.railway.app/sam3-litetext/segment` by default, with `model: "yonigozlan/sam3-litetext-s0"`, `threshold`, `mask_threshold`, and `return_json: true`. Override the backend with `VITE_SAM3_LITETEXT_URL`, the model id with `VITE_SAM3_LITETEXT_MODEL`, the default thresholds with `VITE_SAM3_LITETEXT_THRESHOLD` and `VITE_SAM3_LITETEXT_MASK_THRESHOLD`, and the bearer token with `VITE_SAM3_LITETEXT_CLIENT_TOKEN` or `VITE_API_AUTH_TOKEN`. The frontend expects JSON and can draw a returned overlay image URL/data URL or masks serialized as 2D arrays, flat `{ data, width, height }` objects, or uncompressed COCO-style `{ counts, size }` RLE. It also shows the raw JSON response so PyTorch tensor outputs should be converted by the backend before returning.
