...
### Changelog notes
2026-04-23: Fixed the Jasmine voice reply flow getting stuck at `Jasmine is speaking...` after ASR completed. Configuration impact: the chat transcript now receives the captured plant persona data correctly before starting the AI voice reply request, and the reply endpoint can use `VITE_REPLY_CLIENT_TOKEN`, `VITE_TTS_CLIENT_TOKEN`, or the existing `VITE_ASR_CLIENT_TOKEN` fallback. Verification: run `cd ./examples && npm run start:http`, open the app, enter Jasmine chat, tap the microphone, say a short phrase, and confirm DevTools shows a successful `/reply/voice/stream` request followed by audible Jasmine speech.
